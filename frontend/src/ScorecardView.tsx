  ...
  return <section className="scorecard-page" aria-labelledby="scorecard-title">
    ...
    {scorecard.form_type === 'AdministrativeSupport' ? <div className="table-scroll"><table className="data-table scorecard-table">
      ...
      {lines.map((line, index) => <fieldset className="scorecard-fieldset" key={`${line.id ?? 'new'}-${index}`}>
        ...
        <label>Linked Strategy Reference<select aria-label={`Linked Strategy Reference ${index + 1}`} disabled={!editablePlan} value={line.linkedStrategyReferenceId} onChange={(event) => change(index, 'linkedStrategyReferenceId', event.target.value)}><option value="">Select</option>{strategyReferences.map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}</select></label>
        <label>Measure<input aria-label={`Measure ${index + 1}`} disabled={!editablePlan} value={line.measureDescription} onChange={(event) => change(index, 'measureDescription', event.target.value)} /></label>
        <label>Target<input aria-label={`Target ${index + 1}`} disabled={!editablePlan} value={line.target} onChange={(event) => change(index, 'target', event.target.value)} /></label>
        <label>Weight<input aria-label={`Weight ${index + 1}`} disabled={!editablePlan} type="number" min="0.001" max="100" step="0.001" value={line.weight ?? ''} onChange={(event) => change(index, 'weight', event.target.value === '' ? null : Number(event.target.value))} /></label>
        {scorecard.current_phase === 'MidYear' && <>
          <label>Mid-Year Status<select aria-label={`Mid-Year Status ${index + 1}`} disabled={!editablePlan} value={line.midYearStatus} onChange={(event) => change(index, 'midYearStatus', event.target.value)}><option value="">Select</option><option value="OnTrack">On Track</option><option value="AtRisk">At Risk</option><option value="Blocked">Blocked</option></select></label>
          <label>Mid-Year Comment<textarea aria-label={`Mid-Year Comment ${index + 1}`} disabled={!editablePlan} value={line.midYearComment} onChange={(event) => change(index, 'midYearComment', event.target.value)} /></label>
          <label>Manager Mid-Year Comment<textarea aria-label={`Manager Mid-Year Comment ${index + 1}`} disabled={!managerPending} value={line.managerComment} onChange={(event) => change(index, 'managerComment', event.target.value)} /></label>
        </>}
    ...
    {scorecard.current_phase === 'Development' && <div className="development-fields">
      <label>Employee Development Notes<textarea disabled={!employeePending} value={employeeDevelopmentNotes} onChange={(event) => setEmployeeDevelopmentNotes(event.target.value)} /></label>
      <label>Manager Development Notes<textarea disabled={!managerPending} value={managerDevelopmentNotes} onChange={(event) => setManagerDevelopmentNotes(event.target.value)} /></label>
    </div>}
